...
animdata.chart = function() {
    ...
    var j, k = {
            ...
            age: d3.scale.linear().domain([17, 60]).range([0, 1e3]),
            date: d3.time.scale().domain([new Date("1950-01-01"), new Date("2015-12-01")]).range([0, 1e3]),
            histogram: d3.scale.linear().domain([0, 350]).range([0, 1e3])
        }, n = d3.svg.axis().orient("top").tickSize(-16618),
        o = {
...
animdata.story = function() {
    ...
    var g = {
        container: d3.select(".story")
            ...
            text: "Max Verstappen is by far the youngest ever driver to have competed in Formula 1 making his debut aged 17. He's also become the youngest ever points scorer, scoring points in his second Grand Prix.",
            scrollTo: 0,
            position: [300, 80],
            xType: "age"
        }, {
            ...
            text: "Of the current drivers, Sebastian Vettel has the most wins at 40. His closest competitors are Lewis Hamilton and Fernando Alonso with 34 and 32 wins, respectively. Will they catch him?<br/>Close this box to explore further...",
            scrollTo: 600,
            position: [124, 920],
            xType: "histogram"
        }];
